...
app.get('/make-server-1cd2eafa/surveys', async (c) => {
  try {
    const surveys = await kv.getByPrefix('survey:');
...
app.post('/make-server-1cd2eafa/survey-responses', async (c) => {
  try {
    ...
    await kv.set(responseId, response);
  ...
  try {
    ...
    if (body.sender === 'user') {
      ...
      setTimeout(async () => {
        ...
        await kv.set(policeMessageId, policeMessage);
      }, 1500);
    }
  ...
  try {
    ...
    const surveys = [
      ...
      },
      {
        ...
        type: 'multiple',
        options: ['Robos residenciales', 'Asaltos en la calle', 'Vandalismo', 'Tráfico de drogas', 'Violencia doméstica']
      }
    ];
    ...
    const alerts = [
      {
        ...
        description: 'Varios ciudadanos reportaron movimientos inusuales cerca del parque central. Patrulla policial en camino.',
      ...
      },
      {
        ...
        description: 'Se ha identificado un vehículo abandonado en la zona. Las autoridades están verificando.',
    ...
    await kv.set(welcomeMessage.id, welcomeMessage);
